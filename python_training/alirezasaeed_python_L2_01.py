...
class PasswordValidator:
    ...
    def validate(self) -> None:
        while True:
            ...
            if not any(char in string.ascii_letters for char in self.password):
                print(
                    "2. password must contain at least one a-z or A-Z character.",
                    color="red",
                )
                score -= 1
            ...
            if self.leet_substitution():
                print(
                    "7. Password must not be the same as the username with leet substitutions.",
                    color="red",
                )
                score -= 1
            ...
            print(
                "\n\npassword score for this password is:",
                self.score(score),
                color="magenta",
            )
            print("------------------------------------------------")
            ...
            if score < 6:
                print(
                    "Password is not strong enough. Please try again.", color="yellow"
                )
    ...
    def score(self, score) -> int:
        for length, desc in sorted(
            self._PASSWORD_SCORES, key=lambda x: x[0], reverse=True
        ):
            if score >= length:
                return desc
